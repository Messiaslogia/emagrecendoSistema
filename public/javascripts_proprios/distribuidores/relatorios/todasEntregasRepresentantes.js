...
function adquirirListProdutos() {
    axios.get(`${urlDistri}/todasEntregasRepresentantes?idDistribuidor=${id_user}`)
        .then(resp => {
            ...
            listProdutoRepre.forEach(entrega => {
                somaValor = somaValor + entrega.valor_entrega;
            });
            document.querySelector('#CaseEntregas').innerHTML = `R$ ${somaValor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
