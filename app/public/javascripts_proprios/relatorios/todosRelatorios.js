const urlAllFinanceiro = "http://localhost:3000/apiFinanceiro/todasVendas"
...
function paginas(page){
    ...
    for( i = 1; i <= pageCont; i++){
        containerPagination.innerHTML += `<li class="page-item"><a class="page-link cursor-pointer" onclick="showItens(${i})">${i}</a></li>`
    };
}
        ...
        div_relatorio.innerHTML += `
            <tr>
                ...
                <td class="align-middle text-center text-sm">
                    <p class="text-xs font-weight-bold mb-0">${relatorio.valor}</p>
                </td>
...
function criarModal(numero){
    ...
    axios.get(`http://localhost:3000/apiFinanceiro//consultarRelatorio/${numero}`)
        .then(resp => {
